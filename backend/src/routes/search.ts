...
const searchQuerySchema = z.object({
  ...
  kind: z.enum(['all', 'intents', 'users']).default('all'),
  status: z.enum(['PUBLISHED', 'REALIZED']).optional(),
...
searchRouter.get('/', requireAuthenticatedUser, async (request, response, next) => {
  try {
    ...
    const results = await searchIntentsAndUsers(request.appUser!.id, query.q, query);
