...
const AVATAR_PRESETS = [
  ...
  { id: 'avatar-5', label: 'Retrato Natural', url: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=200&auto=format&fit=crop&q=80' },
  { id: 'avatar-6', label: 'Perfil Profissional', url: 'https://images.unsplash.com/photo-1544723795-3fb6469f5b39?w=200&auto=format&fit=crop&q=80' },
];
...
export function EditProfileModal({ user, onClose, onSaved }: EditProfileModalProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (trimmedName.length > 120) {
      setError('O nome de exibição não pode ultrapassar 120 caracteres.');
      return;
    }
    ...
    if (trimmedAvatar.length > 2048) {
      setError('A URL do avatar não pode ultrapassar 2048 caracteres.');
      return;
    }
    if (trimmedAvatar) {
      try {
