...
# e, se houver SSH funcional para a VM, atualiza também o executor da VM.
# =====================================================================
...
section "Atualizando executor da VM, se SSH estiver disponível"

if ssh -o BatchMode=yes -o ConnectTimeout=8 -o StrictHostKeyChecking=accept-new "${VM_USER}@${VM_HOST}" 'true' >/dev/null 2>&1; then
  echo "SSH OK para ${VM_USER}@${VM_HOST}"
  ssh -o BatchMode=yes -o ConnectTimeout=20 -o StrictHostKeyChecking=accept-new "${VM_USER}@${VM_HOST}" \
  ...
  echo "AVISO: SSH para ${VM_USER}@${VM_HOST} não disponível neste momento."
  echo "Resultado: executor PC atualizado; executor VM não atualizado."
  echo "Quando o SSH estiver disponível, execute novamente este mesmo bootstrap."
fi

section "Resumo"

echo "PC: instalador executado e executor listado."
echo "VM: atualizada somente se o SSH respondeu durante este bootstrap."
