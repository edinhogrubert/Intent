...
SRC_URL="${INTENT_EXECUTOR_PC_URL:-https://raw.githubusercontent.com/edinhogrubert/Intent/docs/contratos-agentes/scripts/executores/intent-executor-PC.sh}"
mkdir -p "$BASE"
exec 9>"$LOCK"
...
for c in curl bash mktemp date chmod cp mv; do command -v "$c" >/dev/null || { echo "ERRO: comando ausente: $c"; exit 1; }; done
