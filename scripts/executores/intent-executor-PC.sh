...
latest_bundle(){
  find "$BASE" "$REPO" -maxdepth 6 -type f -name '*.bundle' 2>/dev/null | xargs -r ls -1t 2>/dev/null | head -n 1
}

valid_bundle(){
  local b="$1"
  [[ -n "$b" && -s "$b" ]] || return 1
  git -C "$REPO" bundle verify "$b" >/dev/null
}

task_1(){
  [[ "$(id -un)" == grubert ]] || fail 1 "usuário inesperado: $(id -un)"
  [[ "$(hostname -s)" == lubuntu ]] || fail 1 "host inesperado: $(hostname -s)"
  for c in bash git find df flock; do command -v "$c" >/dev/null || fail 1 "comando ausente: $c"; done
  [[ -d "$BASE" && -d "$REPO" ]] || fail 1 'paths base/repo ausentes'
...
task_3(){
  ...
  git -C "$REPO" fetch --tags --quiet origin || fail 3 'fetch tags falhou'
...
task_5(){
  need_repo || fail 5 'repo inválido'
  local b; b="$(latest_bundle || true)"
  valid_bundle "$b" || fail 5 "backup Git inválido/ausente: ${b:-nenhum}"
  ok 5 "backup Git válido: $b"
}

task_6(){
  task_4 || fail 6 'verificação de recursos falhou'
  [[ -f "$BACKUP_SCRIPT" ]] || fail 6 "script auxiliar ausente: $BACKUP_SCRIPT"
  bash -n "$BACKUP_SCRIPT" || fail 6 'sintaxe inválida no backup Git'
  bash "$BACKUP_SCRIPT" || fail 6 'backup Git falhou'
  local b; b="$(latest_bundle || true)"
  valid_bundle "$b" || fail 6 "bundle gerado inválido: ${b:-nenhum}"
...
task_7(){
  ...
  git -C "$REPO" fetch origin --prune || fail 7 'fetch falhou'
  git -C "$REPO" checkout main || fail 7 'checkout main falhou'
  git -C "$REPO" pull --ff-only origin main || fail 7 'pull --ff-only falhou'
  git -C "$REPO" log --oneline -5
  ok 7 'main local sincronizada'
}

task_8(){
  need_repo || fail 8 'repo inválido'
  git -C "$REPO" diff --check || fail 8 'git diff --check falhou'
  if has_npm_script "$REPO" lint; then (cd "$REPO" && npm run lint) || fail 8 'lint falhou'; fi
  if has_npm_script "$REPO" build; then (cd "$REPO" && npm run build) || fail 8 'build frontend falhou'; fi
  if [[ -d "$REPO/backend" ]]; then
    if has_npm_script "$REPO/backend" test; then (cd "$REPO/backend" && npm test) || fail 8 'testes backend falharam'; fi
    if has_npm_script "$REPO/backend" build; then (cd "$REPO/backend" && npm run build) || fail 8 'build backend falhou'; fi
  fi
...
task_9(){
  ...
  [[ $miss -eq 0 ]] || fail 9 'continuidade incompleta'
...
task_11(){
  ...
  [[ "${INTENT_AUTORIZO_RELEASE:-}" == SIM ]] || fail 11 'INTENT_AUTORIZO_RELEASE=SIM ausente'
  task_10 || fail 11 'preparação de release falhou'
  git -C "$REPO" tag -a "$INTENT_RELEASE_TAG" -m "Intent $INTENT_RELEASE_TAG" || fail 11 'criação da tag falhou'
  git -C "$REPO" push origin "$INTENT_RELEASE_TAG" || fail 11 'push da tag falhou'
