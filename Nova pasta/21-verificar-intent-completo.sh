...
REMOTE_COMMIT="$(sudo -u ubuntu git -C "${SOURCE_DIR}" ls-remote origin "refs/heads/${BRANCH}" 2>/dev/null | awk '{print $1}')"
REMOTE_CONTAINS_DEPLOYED=0

if [[ -n "${REMOTE_COMMIT}" ]]; then
    sudo -u ubuntu git -C "${SOURCE_DIR}" fetch --quiet origin "${BRANCH}" 2>/dev/null || true
    if sudo -u ubuntu git -C "${SOURCE_DIR}" merge-base --is-ancestor "${EXPECTED_COMMIT}" FETCH_HEAD 2>/dev/null; then
        REMOTE_CONTAINS_DEPLOYED=1
    fi
fi

[[ "${CURRENT_COMMIT}" == "${EXPECTED_COMMIT}" ]] \
    && pass "Servidor posicionado no commit correto." \
    || fail "Commit local ${CURRENT_COMMIT:-ausente}; esperado ${EXPECTED_COMMIT}."

if [[ "${REMOTE_COMMIT}" == "${EXPECTED_COMMIT}" ]]; then
    pass "Branch remota está no mesmo commit implantado."
elif [[ "${REMOTE_CONTAINS_DEPLOYED}" -eq 1 ]]; then
    pass "Branch remota contém o commit implantado e possui atualizações posteriores."
else
    fail "Branch remota não contém o commit implantado ${EXPECTED_COMMIT}."
fi

[[ -z "$(git -C "${SOURCE_DIR}" status --porcelain 2>/dev/null)" ]] \
    && pass "Repositório sem alterações locais." \
    || fail "Existem alterações locais no repositório."
...
docker exec intent-frontend grep -Rqs "Intents das pessoas que você segue" /usr/share/nginx/html \
    && pass "Feed Seguindo está no frontend ativo." \
